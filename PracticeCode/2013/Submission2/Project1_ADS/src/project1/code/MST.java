		...
		if(inputData.length==3){
			...
			if(inputData.length==2){
				...
				if(type.equalsIgnoreCase("-s")){
					...
					System.out.println("Total Time taken for Simple-Scheme (file Input): "+(stopTime-startTime));
					...
					if(type.equalsIgnoreCase("-f")){
						...
						System.out.println("Total Time taken for F-Heap (file Input): "+(stopTime-startTime));
						
					}else{
						System.out.println(args.length+"inou"+inputData.length+" "+(type=="-f")+" "+type);
						printInstruction();
					}			
				}
			}else{
